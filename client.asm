...
;; TCP echo client using x86_64 Linux syscalls
...
_start:
  ...
  .noMoreArgs:
    ...
    bswap edx ; change byte order
    mov eax, edx
    mov dword [pop_sa + sockaddr_in.sin_addr], eax ; store hex IP to structure
  ...
  .mainloop:
    ...
    ; Read user input and send string to the server
    ; Receive the echo and output to screen
    .readloop:
      call _get_msg
      call _echo
      call _read
      ...
      .read_complete: ;; Close server socket
        mov  rdi, [server]
        call _close_sock
        mov  word [server], 0
        jmp  .mainloop
...
;; Establish connection to server and store the new server socket descriptor
_connect:
  mov rax, 42
  mov rdi, [sock]
  mov rsi, pop_sa
...
;; Reads up to 256 bytes from the server into echobuf and sets the read_count variable
...
;; Sends up to the value of read_count bytes from echobuf to the server socket
;; using sys_write
_echo:
  mov rax, 1 ; SYS_WRITE
  mov rdi, [sock] ; client socket fd
  mov rsi, echobuf ; buffer
  mov rdx, [read_count] ; read 256 bytes ; mov rdx, 256 ; number of bytes received in _read
  syscall
...
;; Prompt user for server port number. Sets the port number to the default 22222
...
;; Prompt the user for the message to send to the server
_get_msg:
  mov rax, 1
  mov rdi, 1
